...
function deliverC1C2() {
  $.getJSON('https://a1.cnblogs.com/group/C1-C2', function(n) {
    n && n.C1 ? (n.C1 && $('#cnblogs_c1').html(n.C1), n.C2 ? $('#cnblogs_c2').html(n.C2) : $('#cnblogs_c2').hide()) : $.getScript('https://securepubads.g.doubleclick.net/tag/js/gpt.js', function() {
      deliverGoogleAdC1C2()
    })
  })
}
...
function blogCommentManager() {
  ...
  this.PostNewComment = function() {
    ...
    $.ajax({
      ...
      timeout: 3e4,
      success: function(n) {
        n ? (n.isSuccess ? (ShowCommentMsg('感谢您的回复:) 服务器端执行耗时' + n.duration + '毫秒'), $('#tbCommentBody').val(''), $('#btn_edit_comment').click(), commentEditor.removeComment(), $('#divCommentShow').html($('#divCommentShow').html() + n.message), markdown_highlight('#divCommentShow'), cb_mathjax_render('#divCommentShow'), ResetCommentBox()) : n.message ? ShowCommentMsg(n.message) : ShowCommentMsg('抱歉，评论提交失败！麻烦反馈至 contact@cnblogs.com '), $('#btn_comment_submit').removeAttr('disabled')) : (ShowCommentMsg(errorMsg), $('#btn_comment_submit').removeAttr('disabled'))
      },
      error: function(n, t) {
  ...
  this.UpdateComment = function() {
    ...
    $.ajax({
      ...
      contentType: 'application/json; charset=UTF-8',
      success: function(t) {
        if (t) t.isSuccess ? (ShowCommentMsg('修改成功'), $('#comment_body_' + n.commentId).html(t.message), ResetCommentBox(), commentEditor.removeComment()) : ShowCommentMsg(t.message) else ShowCommentMsg('抱歉！评论修改失败！请与管理员联系(contact@cnblogs.com)。')
      },
      error: function(n) {
  ...
  this.renderComments = function(u, f, e) {
    ...
    o = !1
    s = 0
    typeof e != 'undefined' && (s = e, o = !0)
    !o && u == 0 && location.hash && /^#\d+$/g.test(location.hash) && (o = !0, s = location.hash.substring(1))
    u > 0 && (location.hash = '#!comments')
    h = {postId: cb_entryId, pageIndex: u, anchorCommentId: s}
    ...
    $.ajax({
      ...
      success: function(i) {
        ...
        })) : (n(), cb_CodeHighlight(), $('#blog-comments-placeholder').html(''))
      },
      error: function(t) {
        t.status > 0 && $('#blog-comments-placeholder').html('抱歉！发生了错误！麻烦反馈至contact@cnblogs.com')
        n()
...
function loadBlogNews() {
  $.ajax({
    url: getAjaxBaseUrl() + 'news.aspx', type: 'get', dataType: 'text', success: function(n) {
      if (n) if (n.indexOf('<script') < n.indexOf('<script>getFollowStatus')) {
...
function LoadPostInfoBlock(n, t, i, r) {
  $.ajax({
    ...
    type: 'get',
    contentType: 'application/json; charset=utf-8',
    data: {blogId: n, postId: t, blogUserGuid: r},
    cache: !1,
    dataType: 'text',
    timeout: 1e4,
    success: function(n) {
...
function GetPrevNextPost(n) {
  $.get(getAjaxBaseUrl() + 'post/prevnext', {postId: n}, function(n) {
...
function outFromAggHome() {
  ...
  $.ajax({
    ...
    data: JSON.stringify({blogId: currentBlogId, postId: cb_entryId}),
    type: 'post',
    contentType: 'application/json; charset=utf-8',
    dataType: 'json',
    success: function(t) {
...
function copyCnblogsCode(n) {
  var i = getCnblogsCodeContainer(n), f = getCnblogsCodeText(i), r = document.createElement('div'),
      t = document.createElement('textarea'), u
  $(t).val(f)
  $(t).css('width', $(i).width())
  u = $(i).height() * .8
  u > 600 && (u = 600)
...
function getCnblogsCodeText(n) {
  ...
  if (u) for (r = '', f = 0; f < i.length; f++) r += i[f]
...
function WarpClass(n, t, i, r) {
  var e = document.getElementById(n), u = document.getElementById(t), f = document.getElementById(i)
...
function votePost(n, t, i) {
  ...
  i || (i = !1)
  var r = {postId: +n, voteType: t, isAbandoned: i}
...
function getRecommendedList() {
  var t = {itemId: cb_entryId, itemTitle: $('#cb_post_title_url').text()}, n = $('#under_post_news')
  $.ajax({
    ...
    success: function(t) {
      ...
      if (t && t.length >= 5) {
        ...
        $.ajax({
          url: 'https://a1.cnblogs.com/adunits/t5/nocache', type: 'get', dataType: 'html', success: function(t) {
...
function clickRecomItmem(n) {
  var t = {itemId: n, sourceId: cb_entryId}
  $.ajax({
...
function BigBannerDelivery(n) {
  ...
  this.canShowBanner = function() {
    var n = new Date, t = n > new Date(2020, 3, 19, 0, 0) && n < new Date(2020, 11, 1, 0, 0)
  ...
  this.takeAway = function() {
    var n = $('#big_banner'), t = this.skinName
  ...
  this.getCreative = function(n) {
    ...
    return t.href = r.clickThroughUrl, t.target = '_blank', t.addEventListener('click', function() {
      u.clickAd()
    }, !1), i = document.createElement('img'), i.src = r.imageUrl, i.style.width = n, t.appendChild(i), t
  }
  this.setBannerCookie = function(n) {
    var t, i
    n != 0 && (t = new Date, t.setTime(t.getTime() + n * 6e4), i = '; expires=' + t.toUTCString(), document.cookie = this.cookieName + '=close' + i + '; path=/')
  }
}

var JSON, accountBaseAddress, account, getMarkdownEditor, initCommentEditor, currentDelElement, currentCommentID,
    insertUBB, initBlogSubscribeManager, tagOrderer, initTagsManager, buildVoteNode
JSON || (JSON = {}), function() {
  'use strict'
...
account = {
  login: function(n) {
    var t = location.href
    return n && t.indexOf('#' + n) < 0 && (t += '#' + n), location.href = accountBaseAddress + '/signin?returnUrl=' + encodeURIComponent(t), !1
  }, logout: function() {
    ...
    return location.href = accountBaseAddress + '/signup?returnUrl=' + encodeURIComponent(location.href), !1
  }
}, function() {
...
getMarkdownEditor = function(n) {
  var t = {}
  return t.textbox = document.getElementById(n), t.config = {enableAutoCompletion: !1}, t.replaceSelectionWith = function(n) {
    var i = n.length - t.getSelection().length, r = t.textbox.selectionStart, u = t.textbox.selectionEnd + i
    ...
    var n = t.textbox.selectionStart, i = t.textbox.value.substring(0, n).lastIndexOf('\n')
    ...
    var n = t.textbox.selectionStart, i = t.textbox.selectionEnd
  ...
  }, t.bracketMatchPairs = [{left: '(', right: ')'}, {left: '[', right: ']'}, {left: '{', right: '}'}, {
    left: '``',
    right: '`\n```',
    backspace: '``',
    moveBack: 4,
    ...
    trigger: '*',
    needLeftMatch: !0
  }, {left: '"', right: '"'}], t.bracketMatch = function(n) {
    for (var r = 0; r < t.bracketMatchPairs.length; r++) {
      var i = t.bracketMatchPairs[r], u = t.getStringRelativeToCaret(-i.left.length) === i.left,
  ...
  }, t.hotKeyMap = [{key: 'ctrl+`', action: t.code}, {key: 'ctrl+b', action: t.bold}, {
    key: 'ctrl+q',
    action: t.quote
  }, {key: 'ctrl+k', action: t.link}, {key: 'shift+tab', action: t.decreaseIndent}, {
    key: 'tab',
...
initCommentEditor = function(n) {
  ...
  t.currentEditor.hotKeyMap.push({key: 'ctrl+i', action: t.image})
  t.code = function() {
  ...
  t.preview = function() {
    ...
    $.ajax({
      ...
      timeout: 5e3,
      success: function(n) {
        if (n) $('#tbCommentBodyPreviewBody').html(n.message), markdown_highlight('#tbCommentBodyPreviewBody'), $('#btn_comment_submit').removeAttr('disabled') else $('#tbCommentBodyPreviewBody').html('抱歉！预览加载失败！请与管理员联系(contact@cnblogs.com)。'), $('#btn_comment_submit').removeAttr('disabled')
      },
      error: function(n, t) {
...
$.fn.extend({
  selection: function() {
    ...
    if (t) this.get(0).focus(), t.selection.createRange().collapse, this.get(0).document.selection.createRange().text = n else if (this.get(0).selectionStart || this.get(0).selectionStart == '0') {
      var r = this.get(0).selectionStart, u = this.get(0).selectionEnd, i = this.get(0).value, f = i.substring(0, r),
          e = i.substring(u)
  ...
  return t.rssTag = function() {
    ...
    loginRequest: function() {
      return dialog({
        ...
        button: [{
          value: '注册', callback: function() {
            account.register()
          }
        }, {
          value: '登录', callback: function() {
            account.login()
          }, autofocus: !0
        }],
        statusbar: t.rssTag()
      ...
      return dialog({
        ...
        ok: function() {
          ...
          n.show()
          $.ajax({
            url: getAjaxBaseUrl() + 'blogSubscription', type: 'POST', dataType: 'JSON', success: function(i) {
              n.close().remove()
      ...
      return dialog({
        ...
        ok: function() {
          ...
          n.show()
          $.ajax({
            url: getAjaxBaseUrl() + 'blogSubscription', type: 'DELETE', success: function(i) {
              n.close().remove()
      ...
      return dialog({
        content: n, cancelValue: '关闭', cancel: function() {
        }, quickClose: !0, statusbar: t.rssTag()
      })
    }, success: function(n) {
      return dialog({
        content: n, ok: function() {
        }, okValue: '确定', quickClose: !0
      })
    }, loading: function() {
      return dialog({calcel: !1})
    }
      ...
      loadLink(location.protocol + '//common.cnblogs.com/scripts/artDialog/ui-dialog.css', function() {
        loadScript(location.protocol + '//common.cnblogs.com/scripts/artDialog/dialog-plus-min.js', function() {
          ...
          return $.ajax({
            ...
            dataType: 'json',
            success: function(i) {
              i ? (n.close(), i.g.isAuthenticated ? i.g.isBlogOwner ? t.dialogs.error('园友可以通过此链接订阅您的博客').show() : i.isSubscribed ? t.unsubscribe() : t.subscribe() : t.login()) : t.dialogs.error('发生错误！').show()
            },
            error: function(n) {
...
initTagsManager = function() {
  ...
  return n.tags = $('#MyTag1_dtTagList td').get(), n.generateTagList = function() {
    ...
    var r = $(n).children('.small').text().slice(1, -1), u = $(t).children('.small').text().slice(1, -1), i = r - u
...
$.ajaxSetup({type: 'post', dataType: 'json', contentType: 'application/json; charset=utf-8', cache: !0})
